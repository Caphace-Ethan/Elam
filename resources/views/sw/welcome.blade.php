    @extends('layouts.app-s')

    @section('content')
  <!--Carousel Wrapper-->
<div id="carousel-example-2" class="carousel slide carousel-fade" data-ride="carousel">
  <!--Indicators-->
  <ol class="carousel-indicators">
    <li data-target="#carousel-example-2" data-slide-to="0" class="active"></li>
    <li data-target="#carousel-example-2" data-slide-to="1"></li>
    <li data-target="#carousel-example-2" data-slide-to="2"></li>
  </ol>
  <!--/.Indicators-->
  <!--Slides-->
  <div class="carousel-inner" role="listbox">
    <div class="carousel-item active">
      <div class="view">
        <img class="d-block w-100" src="images/img%20(68).jpg"
          alt="First slide">
        <div class="mask rgba-black-light"></div>
      </div>
      <div class="carousel-caption">
        <h4 class="h4-responsive">Karibu Elam Christian Harvest Seminary</h4>
        <p>Timiza Ndoto zako</p>
      </div>
    </div>
    <div class="carousel-item">
      <!--Mask color-->
      <div class="view">
        <img class="d-block w-100" src="images/slider(2).jpg"
          alt="Second slide">
        <div class="mask rgba-black-strong"></div>
      </div>
      <div class="carousel-caption">
        <h4 class="h4-responsive">Soma na Uhitimu Elimu yako</h4>
        <p>Kuza Uelewa wako wa Biblia</p>
      </div>
    </div>
    <div class="carousel-item">
      <!--Mask color-->
      <div class="view">
        <img class="d-block w-100" src="images/img%20(9).jpg"
          alt="Third slide">
        <div class="mask rgba-black-slight"></div>
      </div>
      <div class="carousel-caption">
        <h4 class="h4-responsive">Njoo Tujifunze pamoja</h4>
        <p>JIfuze zaidi maarifa ya Biblia</p>
      </div>
    </div>
  </div>
  <!--/.Slides-->
  <!--Controls-->
  <a class="carousel-control-prev" href="#carousel-example-2" role="button" data-slide="prev">
    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
    <span class="sr-only">Previous</span>
  </a>
  <a class="carousel-control-next" href="#carousel-example-2" role="button" data-slide="next">
    <span class="carousel-control-next-icon" aria-hidden="true"></span>
    <span class="sr-only">Next</span>
  </a>
  <!--/.Controls-->
</div>
<!--/.Carousel Wrapper-->
</div>
       <br>
    <div class="container">
        <div class="row"> 
            <div style="color: black;font-family: 'Montserrat', 'Arial', sans-serif;text-align: left;font-size: 14px" class="col-md-4">
                <h6>Matangazo na Habari Mpya</h6>
                 
                    <p><img src="/images/new.gif" width="30px"><a href="/sw/infonews#pills-home"style="color: darkblue;">Angalia Kozi mbalimbali za </a> Astashahada, Stashahada na Shahada</p>
                    <p><img src="/images/new.gif" width="30px"><a href="/sw/infonews"style="color: darkblue;">Mahafali ya 5, yatafanyika Disemba 15,2019</a> kwa wanafunzi walionza 2015/2016  </p>
                    <p><img src="/images/new.gif" width="30px"><a href="/sw/infonews"style="color: darkblue;">Join Various Programmes Conducted by ECHASE</a> </p>
                    <p><img src="/images/new.gif" width="30px"><a href="/sw/infonews"style="color: darkblue;">Various Courses Offered by ECHASE</a> </p>
                    <p><img src="/images/new.gif" width="30px"><a href="/sw/infonews"style="color: darkblue;">Admission Regulations and Requirements</a> for ertificate,Ordinary Diploma  and Bachelor</p>
                    <p><img src="/images/new.gif" width="30px"><a href="/sw/infonews"style="color: darkblue;">Prospectus 2019/2020</a> for Certificate,Ordinary Diploma  and Bachelor </p>
                    <p><img src="/images/new.gif" width="30px"><a href="/sw/infonews"style="color: darkblue;">Fee Structure 2019/2020</a> for Certificate, Ordinary Diploma and Bachelor </p>
                    <p><img src="/images/new.gif" width="30px"><a href="/sw/infonews"style="color: darkblue;">Examination Regulations 2019/2020</a> for Certificate, Ordinary Diploma and Bachelor </p>

              
            </div>
            <div class="col-md-4 ">
                <h6>Muhimu</h6>
                 <div class="col-md-12 col-form-label"> <br>
                <div class="card">
                    <div style="color:white;background-color:#256EC1;" class="card-header">Anza Kozi yako Leo </div>

                    <div class="card-body">
                        <div class="row">
                            <div class="col col--6" style="border-right:dashed;border-right-color:orange ">
                            
                                <a href="" class="btn btn-success">Anza sasa</a>
                                <p>Anza safari yako ya kusoma hapa</p>
                            </div>
                            <div class="col col--6" >
                             <a class="btn btn-primary">Omba taarifa</a>
                              <p>Pata taarfifa zaidi</p>
                              </a>
                            </div>
                        </div>
                                                
                    </div>
                </div>
            </div>
            </div>
            <div class="col-md-4" style="font-size: 14px">
                <h6> Link za Muhimu & Taarifa</h6>
                <ul>
                <li><a href=""style="color: darkblue;">Apply Online for Bachelor Programmes</a> </li>
                <li><a href=""style="color: darkblue;">Apply Online for Ordinary Diploma Programmes</a></li>
                <li><a href=""style="color: darkblue;">Apply Online for Certificates Programmes</a></li>
                <li><a href=""style="color: darkblue;">Echase EOMS</a></li>
                <li><a href="/regulations"style="color: darkblue;">Examination Regulations</a></li>
                <li><a href="/prospectus"style="color: darkblue;">Prospectus 2019/2020</a></li>
                 <li><a href="financial"style="color: darkblue;">Fee Structure 2019/2020</a></li>
                <li><a href=""style="color: darkblue;">Echase Library</a></li>
                </ul>
            </div>

        </div>
        <br>
               <div class="row justify-content-center"> <br>
               <div class="col-md-4 col-form-label"style="border-top:solid;border-top-color:#ffbb3373"> <br>
                <div class="card">
                    <div class="card-header">Karibu Elam Christian Harvest Seminary (ECHASE)</div>

                    <div class="card-body">
                        
                          
                    </div>
                </div>
            </div>
            <div class="col-md-8"style="border-top:solid;border-top-color:#ffbb3373">
                <div class="container">
                
                <div id="multi-item-example" class="carousel slide carousel-multi-item" data-ride="carousel">

      <!--Controls-->
      <div class="controls-top">
        <a class="btn-floating" href="#multi-item-example" data-slide="prev"><i class="fas fa-chevron-left"></i></a>
        <a class="btn-floating" href="#multi-item-example" data-slide="next"><i
            class="fas fa-chevron-right"></i></a>
      </div>
      <!--/.Controls-->

      <!--Indicators-->
      <ol class="carousel-indicators">
        <li data-target="#multi-item-example" data-slide-to="0" class="active"></li>
        <li data-target="#multi-item-example" data-slide-to="1"></li>
        <li data-target="#multi-item-example" data-slide-to="2"></li>
      </ol>
      <!--/.Indicators-->

      <!--Slides-->
      <div class="carousel-inner" role="listbox">

        <!--First slide-->
  <div class="carousel-inner" role="listbox">

        <!--First slide-->
        <div class="carousel-item active">
      <div class="row">
          <div class="col-md-4">
            <div class="card mb-2">
              <img class="card-img-top"
                src="images/img%20(34).jpg"
                alt="Card image cap">
              <div class="card-body">
                <h5 class="card-title">ECHASE Something 01</h5>
                <p class="card-text">Some quick text .............................................
                .......................................................</p>
                <a class="btn btn-primary">Read More..</a>
              </div>
            </div>
          </div>

          <div class="col-md-4">
            <div class="card mb-2">
              <img class="card-img-top"
                src="images/img%20(18).jpg"
                alt="Card image cap">
              <div class="card-body">
                <h5 class="card-title">ECHASE Something 02</h5>
                <p class="card-text">Some quick text .............................................
                .......................................................</p>
                <a class="btn btn-primary">Read More..</a>
              </div>
            </div>
          </div>

          <div class="col-md-4">
            <div class="card mb-2">
              <img class="card-img-top"
                src="images/img%20(35).jpg"
                alt="Card image cap">
              <div class="card-body">
                <h5 class="card-title">ECHASE Something 03</h5>
                <p class="card-text">Some quick text .............................................
                .......................................................</p>
                <a class="btn btn-primary">Read More..</a>
              </div>
            </div>
          </div>

      </div>
    </div>
        <!--/.First slide-->

        <!--Second slide-->
        <div class="carousel-item">
         <div class="row">
          <div class="col-md-4">
            <div class="card mb-2">
              <img class="card-img-top"
                src="images/img%20(60).jpg" alt="Card image cap">
              <div class="card-body">
                <h5 class="card-title">ECHASE Something 04</h5>
                <p class="card-text">Some quick text .............................................
                .......................................................</p>
                <a class="btn btn-primary">Read More..</a>
              </div>
            </div>
          </div>

          <div class="col-md-4">
            <div class="card mb-2">
              <img class="card-img-top"
                src="images/img%20(47).jpg" alt="Card image cap">
              <div class="card-body">
                <h5 class="card-title">ECHASE Something 05</h5>
                <p class="card-text">Some quick text .............................................
                .......................................................</p>
                <a class="btn btn-primary">Read More..</a>
              </div>
            </div>
          </div>

          <div class="col-md-4">
            <div class="card mb-2">
              <img class="card-img-top"
                src="images/img%20(48).jpg" alt="Card image cap">
              <div class="card-body">
                <h5 class="card-title">ECHASE Something 06</h5>
                <p class="card-text">Some quick text .............................................
                .......................................................</p>
                <a class="btn btn-primary">Read More..</a>
              </div>
            </div>
          </div>
    </div>
        </div>
        <!--/.Second slide-->

        <!--Third slide-->
        <div class="carousel-item">
    <div class="row">
          <div class="col-md-4">
            <div class="card mb-2">
              <img class="card-img-top"
                src="images/img%20(53).jpg" alt="Card image cap">
              <div class="card-body">
                <h5 class="card-title">ECHASE Something 07</h5>
                <p class="card-text">Some quick text .............................................
                .......................................................</p>
                <a class="btn btn-primary">Read More..</a>
              </div>
            </div>
          </div>

          <div class="col-md-4">
            <div class="card mb-2">
              <img class="card-img-top"
                src="images/img%20(45).jpg" alt="Card image cap">
              <div class="card-body">
                <h5 class="card-title">ECHASE Something 08</h5>
                <p class="card-text">Some quick text .............................................
                .......................................................</p>
                <a class="btn btn-primary">Read More..</a>
              </div>
            </div>
          </div>

          <div class="col-md-4">
            <div class="card mb-2">
              <img class="card-img-top"
                src="images/img%20(51).jpg" alt="Card image cap">
              <div class="card-body">
                <h5 class="card-title">ECHASE Something 09</h5>
                <p class="card-text">Some quick text .............................................
                .......................................................</p>
                <a class="btn btn-primary">Read More..</a>
              </div>
            </div>
          </div>
    </div>
        </div>
        <!--/.Third slide-->

      </div>
      <!--/.Slides-->

    </div>
    </div>
            </div>
    </div>
        <!-- Test -->

        <!-- End Test-->
    @endsection           

